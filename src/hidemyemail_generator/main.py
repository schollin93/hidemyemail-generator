...
@click.option(
    ...
    help="Keep running and attempt generation every N minutes; 0 runs once",
)
def generatecommand(
    count: int,
    label: str,
...
async def _generate_loop(
    ...
    # Cadence is attempt-start to attempt-start. Attempts that hit the iCloud
    # creation limit just fail and are retried on the next cycle, so a cadence
    # faster than the quota refill never wastes a creation opportunity.
    ...
    total_generated = 0
    console.log(
        f"Loop mode: attempting {count} email(s) every {every_minutes:g} minute(s), "
        f"Ctrl+C to stop"
    )
    try:
        while True:
            ...
            console.rule(f"Cycle {cycles}")
            try:
                # A fresh client per cycle re-reads the cookie file, so a
                # replaced cookies.txt is picked up without restarting.
                emails = await _generate(
                    label,
                    count,
                    cookie_file,
                    output_file,
                ...
                # Keep the loop alive across transient failures (network down,
                # locked database, ...); the next cycle retries from scratch.
                generated = 0
                console.log(f"[bold red][ERR][/] Cycle {cycles} failed: {exc}")

            total_generated += generated
            if generated < count:
                incomplete_cycles += 1

            next_attempt_at = cycle_started + interval_seconds
            while True:
                remaining = next_attempt_at - time.monotonic()
                if remaining <= 0:
                    break
                next_stamp = (
                    datetime.datetime.now() + datetime.timedelta(seconds=remaining)
                ).strftime("%H:%M:%S")
                console.log(
                    f"Running: {total_generated} generated in {cycles} cycle(s), "
                    f"{incomplete_cycles} incomplete. Next attempt at {next_stamp} "
                    f"(in {_format_duration(remaining)})."
                )
                await asyncio.sleep(min(LOOP_HEARTBEAT_SECONDS, remaining))
    finally:
        # Runs on Ctrl+C (task cancellation) as well, so the user always gets
        # a final tally.
        console.log(
            f"Loop stopped after {cycles} cycle(s): "
            f"{total_generated} email(s) generated"
        )
